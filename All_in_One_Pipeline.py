...
# Zwingt das Skript, den Kandidaten zurückzusetzen
FORCE_RETRY_LIST = ["02_BaMg3H8"]
...
# Korrekte feste Pfade aus deinem alten funktionierenden Skript
...
def print_error_log(output_file, label="QE ERROR LOG"):
    ...
    try:
        ...
        snippet   = err_lines[-50:] # Letzte 50 Zeilen
        print(f"      --- {label} ---")
        print("      " + "\n      ".join(snippet))
        print("      " + "-" * 20)
    except: pass
    ...
    try:
        ...
        for input_file in input_files:
            ...
            try:
                ...
                # FIX 2: Trennung von Log-Ausgabe und Daten-Datei!
                ...
                # FIX 1: Append-Falle umgehen. re.findall nutzen und den LETZTEN Wert nehmen!
                e_fermi = "-"
                if os.path.exists(scf_out):
                    with open(scf_out, 'r', errors='ignore') as f:
                        content = f.read()
