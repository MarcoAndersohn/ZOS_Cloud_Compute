...
def ensure_gitignore():
    ...
    if os.path.exists(gi_path):
        with open(gi_path, 'r') as f: existing = f.read().splitlines()
    with open(gi_path, 'a') as f:
        for r in rules:
            if r not in existing: f.write(r + "\n")
...
def print_error_tail(log_file, lines=100):
    ...
    try:
        with open(log_file, 'r', errors='ignore') as f: tail = f.readlines()[-lines:]
        err = f"\n      --- LETZTE {lines} ZEILEN VON {os.path.basename(log_file)} ---\n" + "".join([f"      {l.rstrip()}\n" for l in tail])
        print(err)
        with open(TXT_LOG_FILE, 'a') as f_out: f_out.write(err)
    except: pass

def berechne_tc(omega_log_K, lambda_ep, mu_star=0.13):
    try:
        lam, wlog = float(lambda_ep), float(omega_log_K)
        if lam <= 0 or (lam - mu_star * (1.0 + 0.62 * lam)) <= 0: return 0.0
...
def update_csv(name, status, e_fermi="-", dos_val="-", is_metal="-", min_f="-", stab="-", lam="-", wlog="-", tc="-"):
    ...
    for row in rows:
        if row['Name'] == name:
            ...
            found = True; break
    if not found:
        rows.append({'Name': name, 'Status': status, 'Fermi Energie (eV)': str(e_fermi), 'DOS @ Fermi': str(dos_val), 'Metall?': str(is_metal), 'Min Freq (THz)': str(min_f), 'Stabilität': str(stab), 'Lambda': str(lam) if lam!="-" else "", 'Omega_log (K)': str(wlog) if wlog!="-" else "", 'Tc (K)': str(tc) if tc!="-" else "", 'Timestamp': datetime.now().strftime("%Y-%m-%d %H:%M")})
...
def manage_rolling_checkpoints(work_dir):
    ...
    bkp1, bkp2 = os.path.join(work_dir, "tmp_SAFE_1"), os.path.join(work_dir, "tmp_SAFE_2")
    ...
    for bkp in [os.path.join(work_dir, "tmp_SAFE_1"), os.path.join(work_dir, "tmp_SAFE_2")]:
        if os.path.exists(bkp):
            ...
            try:
                ...
                if is_xml_valid(os.path.join(tmp_dir, f"{prefix}.save", "data-file-schema.xml")): return True
            except: pass
    return False
...
def make_kpoints_dense(filepath):
    ...
    for line in content.split('\n'):
        if "K_POINTS" in line.upper() and "automatic" in line.lower(): in_k = True; out_lines.append(line); continue
...
def analyze_crash_reason(output_file, start_size=0):
    ...
    try:
        ...
        if "wrong trans" in lines or "wrong elph" in lines: return "WRONG_TRANS_ERROR"
        ...
        if ram_match:
            if "self-consistent calculation" not in lines and "iteration #" not in lines: return "LIKELY_OOM"
...
def run_monitored_pw(input_file, output_file, cwd, active_cores):
    ...
    content = re.sub(r"pseudo_dir\s*=\s*['\"].*['\"]", f"pseudo_dir='{PSEUDO_DIR}/'", content) if "pseudo_dir" in content else content.replace("&CONTROL", f"&CONTROL\n pseudo_dir='{PSEUDO_DIR}/',")
    content = re.sub(r"electron_maxstep\s*=\s*\d+", "electron_maxstep = 150", content) if "electron_maxstep" in content else content.replace("&ELECTRONS", "&ELECTRONS\n electron_maxstep = 150,")
    
    tmp_dir = os.path.join(cwd, "tmp") 
    prefix = re.search(r"prefix\s*=\s*['\"]([^'\"]+)['\"]", content).group(1) if re.search(r"prefix\s*=\s*['\"]([^'\"]+)['\"]", content) else "calc"
    ...
    if mode == 'from_scratch' and os.path.exists(tmp_dir): shutil.rmtree(tmp_dir, ignore_errors=True)
    ...
    with open(input_file+".run", 'r') as f_in, open(output_file, 'a' if mode=='restart' else 'w') as f_out:
        ...
        try:
            while process.poll() is None: time.sleep(2)
...
def execute_scf_block(name, scf_in, scf_out, work_dir, phase_label):
    if os.path.exists(scf_out) and "JOB DONE" in open(scf_out, errors='ignore').read():
        if not os.path.exists(os.path.join(work_dir, "tmp")): os.remove(scf_out)
        else: return "DONE", int(DEFAULT_CORES)
...
def run_monitored_ph(input_file, output_file, cwd, active_cores):
    ...
    with open(input_file, 'r') as f: content = f.read()
    if "recover=" in content.lower(): content = re.sub(r"recover\s*=\s*\.[a-zA-Z]+\.", "recover=.true.", content, flags=re.IGNORECASE)
    else: content = content.replace("&INPUTPH", "&INPUTPH\n recover=.true.,")
    if "max_seconds" in content.lower(): content = re.sub(r"max_seconds\s*=\s*\d+", "max_seconds=7200", content, flags=re.IGNORECASE)
    else: content = content.replace("&INPUTPH", "&INPUTPH\n max_seconds=7200,")
    ...
    with open(input_file+".run", 'r') as f_in, open(output_file, file_mode) as f_out:
        ...
        if process.returncode == -9: return "OOM", run_duration
        ...
        try:
            with open(output_file, 'rb') as f:
                ...
                if "JOB DONE" in out_new:
                    if "STOP" in out_new or "max_seconds" in out_new.lower(): return "TIMEOUT_STOP", run_duration
...
def main():
    ...
    with open(TXT_LOG_FILE, "a") as f: f.write(f"\n\n{'='*40}\n🚀 NEUSTART SMART-PIPELINE\n{'='*40}\n")
    print(f"\n\n{'='*40}\n🚀 NEUSTART SMART-PIPELINE\n{'='*40}\n")
    ...
    for input_file in input_files:
        ...
        prefix = re.search(r"prefix\s*=\s*['\"]([^'\"]+)['\"]", open(scf_in).read()).group(1) if re.search(r"prefix\s*=\s*['\"]([^'\"]+)['\"]", open(scf_in).read()) else "calc"
        ...
        if not os.path.exists(ph_out) or "JOB DONE" not in open(ph_out, errors='ignore').read():
            ...
            if not os.path.exists(pristine) and os.path.exists(os.path.join(work_dir, "tmp")): shutil.copytree(os.path.join(work_dir, "tmp"), pristine)
            ...
            while ph_attempts < 5:
                ...
                if ph_res == "TIMEOUT_STOP":
                    print("      ⏱️ Zeitlimit (2h) erreicht. Setze nahtlos fort...")
                    ph_attempts -= 1; continue
                
                if ph_res in ["CRASH", "XML_ERROR", "ELPH_CORRUPT"] and run_duration < 60:
                    print("      🧨 Sofortiger Crash! Phonon-Dateien sind vergiftet. Lösche _ph0...")
                ...
                print(f"      ⚠️ Stop in Phononen (Grund: {ph_res}). Lade Backup...")
