...
def kill_process_tree(pid):
    ...
    try:
        ...
        print(msg)
        with open(TXT_LOG_FILE, "a") as f_log:
            f_log.write(msg + "\n")
    except psutil.NoSuchProcess:
        pass
...
def update_csv(name, status, e_fermi="-", dos_val="-", is_metal="-", min_f="-", stab="-", lam="-", wlog="-", tc="-"):
    ...
    if not found:
        ...
        if lam != "-":
            new_row['Lambda'] = str(lam)
        if wlog != "-":
            new_row['Omega_log (K)'] = str(wlog)
        if tc != "-":
            new_row['Tc (K)'] = str(tc)
        rows.append(new_row)
...
def analyze_crash_reason(output_file):
    ...
    try:
        ...
        if ram_match and "self-consistent calculation" not in lines and "iteration #" not in lines:
            return "LIKELY_OOM"
...
def run_monitored_ph(input_file, output_file, cwd, active_cores):
    ...
    if "recover=.true." in content.lower():
        file_mode = 'a'
    else:
        file_mode = 'w'
...
def main():
    ...
    for input_file in input_files:
        ...
        if os.path.exists(scf_out) and "JOB DONE" in open(scf_out, errors='ignore').read():
            ...
            # Überprüfen ob SCF erfolgreich, aber der tmp Ordner beschädigt oder gelöscht wurde
            if not os.path.exists(os.path.join(work_dir, "tmp")):
                msg_tmp = "   ⚠️ SCF.out existiert, aber tmp-Ordner fehlt. Lösche scf.out für sauberen Neustart."
                print(msg_tmp)
                with open(TXT_LOG_FILE, "a") as f_log:
                    f_log.write(msg_tmp + "\n")
                os.remove(scf_out)
