...
def print_error_log(output_file, label="QE ERROR LOG", lines=15):
    if not os.path.exists(output_file): return
    try:
        err_lines = open(output_file, errors='ignore').read().strip().split('\n')
        snippet   = err_lines[-lines:]
        print(f"      --- {label} ---")
        print("      " + "\n      ".join(snippet))
        print("      " + "-" * 20)
    except: pass
...
def apply_oom_settings(input_file, level, force_cg=False):
    ...
    if level >= 1 or force_cg: diag = 'cg'; mix = 4; disk = 'low'; msg = "Stufe 1 (cg, mix=4, disk_io='low')"
...
def run_phonon_block(name, work_dir, scf_in, scf_out, ph_in, ph_out, e_fermi, dos_val):
    ...
    if not already_stable:
        ...
        with open(ph_out, 'r') as f:
            ...
            if freqs:
                min_f_val = min(float(x) for x in freqs)
                stab = "STABIL" if min_f_val > -0.05 else "INSTABIL"

        min_f_str = f"{min_f_val:.4f}" if min_f_val is not None else "-"

        if stab == "INSTABIL":
            print(f"   🛑 Material ist INSTABIL (Min Freq, {min_f_str} THz). Überspringe El-Ph.")
            update_csv(name, "Fertig (Metall)", e_fermi, round(dos_val, 4), "JA", min_f=min_f_str, stab=stab)
            cleanup_heavy_files(work_dir, name)
            git_sync(f"Fertig, {name} (INSTABIL)")
            return "DONE"
        
        print(f"   ✅ Material ist STABIL (Min Freq, {min_f_str} THz). Gehe zu Phase 2...")
        update_csv(name, "Fertig (Metall)", e_fermi, round(dos_val, 4), "JA", min_f=min_f_str, stab=stab)
    ...
    if not is_resuming_phase2:
        print(f"   ⚛️ Erster Start von Phase 2 für {name}, behalte Cache für Interpolation...")
        # WICHTIG: Hier keine Cache-Daten (_ph0) mehr löschen
        if os.path.exists(ph_out): os.remove(ph_out)
    ...
    try:
        ...
        for input_file in input_files:
            ...
            try:
                ...
                dos_crash = analyze_crash_reason(dos_out)
                if dos_crash == "XML_ERROR" or dos_crash == "WF_COLLECT_ERROR":
                    ...
                    if os.path.exists(scf_out): os.remove(scf_out)
                    if os.path.exists(dos_out): os.remove(dos_out)
